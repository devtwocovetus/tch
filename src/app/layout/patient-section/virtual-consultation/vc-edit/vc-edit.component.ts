...
export class VcEditComponent implements OnInit {
	...
	EditPatientInfo(status){
		...
		console.log(this.getSlotsData, this.bookingInfo.value.VCB_Doctor_Name)
		if(this.bookingInfo.value.VCB_Doctor_Name == ''){
			this.toster.warning('Please select doctor','Warning')
			return
		}
		if(this.getSlotsData.timeslot == ''){
		...
		this.bookingInfo.value.VCB_Status = status
		...
		Observable.forkJoin([booking, patientedit]).subscribe(results =>{
			...
			this.router.navigateByUrl('/patient-vc-list')
		})
	}
	checkSSNData(ssn){
		console.log(ssn)
