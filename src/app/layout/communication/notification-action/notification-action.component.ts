...
@Component({
	...
	styleUrls: ['./notification-action.component.css']
})
export class NotificationActionComponent implements OnInit {
	...
	sendvoicemassage(){
		...
		if (($("#mycontact").val().length) < 6) {
			this.toster.warning(this.translate.instant('length should be greater then six digit'), this.translate.instant('Warning'))
			return 
		}
		if(this.reqData.mycontact == '' || this.reqData.mycontact == null || this.reqData.mycontact == undefined)
		{
