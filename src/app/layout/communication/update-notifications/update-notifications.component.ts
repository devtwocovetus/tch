...
@Component({
	...
	styleUrls: ['./update-notifications.component.css']
})
export class UpdatenotificationsComponent implements OnInit {
	...
	getDataViaId(){
		...
		this.UserService.getSingleNotifications(obj).subscribe(data =>{
			console.log(data);
			this.reqData.NC_Category =	data.Data.NFT_Category_ID
			this.reqData.NC_Type = data.Data.NFT_Category_Type_ID
			this.reqData.name =	data.Data.NFT_Name 
			this.reqData.messages = data.Data.NFT_Description
			this.changeCat('w')
...
GetNotiCategoryList(){
  ...
  goBack(){
	...
	finalAction(){
		...
		console.log(obj)
		this.UserService.updateNotifications(obj).subscribe((data)=>{
			console.log(data)
			 this.ngOnInit()
		},err=>{
			console.log(err)
