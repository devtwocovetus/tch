...
export class EditIntakeComponent implements OnInit {
	...
	getDataViaId(){
		...
		this.UserService.getSingleIntakeActons(obj).subscribe(data =>{
			console.log(data.Data)
			this.NotiActions = data.Data.PIT_Actions
			...
			if(this.NotiActions[0].NFA_Action_Type == 'Email')
			{
				...
				this.reqData.dayandweeks = this.NotiActions[0].NFA_DayOrWeek
				console.log(this.reqData.emailsubject)
			}
	...
	editnotificatinsactions(){
		...
		var Noti = {
			...
			PIT_Unique_ID: this.GetNotificationid,    
			PIT_Actions: this.dummyArrayForNow,                       
		...
		console.log(Noti)
		this.showLoader = true
		this.UserService.editPatientIntakeActons(Noti).subscribe((data1)=>{
