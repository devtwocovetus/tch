	...
	ngOnInit() {
		...
		if(this.setAccToSurgery){
			if(this.setAccToSurgery.PhyO_DBA_Name){
				this.title.setTitle('Walk-in - ' +  this.setAccToSurgery.PhyO_DBA_Name)
			}else if(this.setAccToSurgery.SurgC_DBA_Name){
				this.title.setTitle(' Walk-in - ' +  this.setAccToSurgery.SurgC_DBA_Name)
			}
		}else{
			this.title.setTitle('Walk-in - The Cloud Health')
		}
	...
	finalAction(){
		...
		this.UserService.addPatient(akObj).subscribe((PatientResponse)=>{
			...
			this.UserService.addBooking(BookingObj).subscribe((BookingResponse)=>{
				...
				var jsdom = JSON.stringify(BookingResponse)

				console.log(JSON.parse(jsdom));
